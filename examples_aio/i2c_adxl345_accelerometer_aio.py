...
async def adxl345(my_board):
    # setup adxl345
    # device address = 83
    await my_board.set_pin_mode_i2c(0)

    # set up power and control register
    await my_board.i2c_write(83, [45, 0], i2c_port=0)
    await asyncio.sleep(.1)
    await my_board.i2c_write(83, [45, 8], i2c_port=0)
    await asyncio.sleep(.1)

    # set up the data format register
    await my_board.i2c_write(83, [49, 8], i2c_port=0)
    await asyncio.sleep(.1)
    await my_board.i2c_write(83, [49, 3], i2c_port=0)
    await asyncio.sleep(.1)
    ...
    while True:
        ...
        try:
            await my_board.i2c_read(83, 50, 6, callback=the_callback, i2c_port=0)
            await asyncio.sleep(.1)
